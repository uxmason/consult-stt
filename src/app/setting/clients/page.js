...
export default function Home() {
  ...
  const boostListData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/clients/?index=${indexBoard}`;
      if(indexBoard == null) url = `/api/setting/clients/`;
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C05}>
              <div className={'C02 isOpacity isMini isUnderline isShort'}>
                <p className={'T05 P20'}>회원번호</p>
                <p className={'T05 P30'}>이름</p>
                <p className={'T05 P20'}>주민번호</p>
                <p className={'T05 P30'}>지점명</p>
              </div>
              <div className={'C05'}>
                {data && data.map((item, index) => {
                  return <div key={index} className={'C02 isPrevented'}>
                    <p className={'T05 P20'}>{item.PSENTRY}</p>
                    <p className={'T05 P30'}>{item.PSNAME}</p>
                    <p className={'T05 P20'}>{item.LICENSE}</p>
              ...
              <div className={'C04'}>
                <p className={'T06 styleSheet isFirst'} onClick={e => {
                  router.push(`/setting/clients/`, {scroll: false});
                  setIndexBoard(1);
                }}></p>
                <p className={'T06 styleSheet isPrev'} onClick={e => {
                  if(indexBoard > 10) {
                    router.push(`/setting/clients/?index=${Math.floor((indexBoard - 1) / 10)*10}`, {scroll: false});
                    setIndexBoard(Math.floor((indexBoard - 1) / 10)*10);
                  } else {
                    router.push(`/setting/clients/?index=1`, {scroll: false});
                    setIndexBoard(1);
                  }
                }}></p>
                <div className={'C05'}>
                  {boardNumbers.map((item, index) => {
                    return (<p key={index} className={`T06 isPager ${(indexBoard == item.number)? `isSelected`:null}`} onClick={e => {
                      router.push(`/setting/clients/?index=${item.number}`, {scroll: false});
                      setIndexBoard(item.number);
                    }}>{item.number}</p>)
                  })}
                </div>
                {Math.ceil(indexBoard / 10) != Math.ceil(totalCount / 100) ? <><p className={'T06 styleSheet isNext'} onClick={e => {
                  router.push(`/setting/clients/?index=${Math.ceil(indexBoard / 10)*10 + 1}`, {scroll: false});
                  setIndexBoard(Math.ceil(indexBoard / 10)*10 + 1);
                }}></p><p className={'T06 styleSheet isEnd'} onClick={e => {
                  router.push(`/setting/clients/?index=${Math.ceil(totalCount / 10)}`, {scroll: false});
