...
export default function SettingAdmins() {
  return (
...
function Content() {
  ...
  const boostListData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/admins/?index=${indexBoard}`;
      if(indexBoard == null) url = `/api/setting/admins/`;
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C05}>
              <div className={'C02 isOpacity isMini isUnderline isShort'}>
                <p className={'T05 P20'}>ID</p>
                <p className={'T05 P25'}>직급/직책</p>
              ...
              <div className={'C05'}>
                {data && data.map((item, index) => {
                  return <div key={index} className={'C02 isPrevented'}>
                    <p className={'T05 P20'}>{item.ADMIN_ID}</p>
                    <p className={'T05 P25'}>{item.RANK_NAME} / {item.POSITION_NAME}</p>
                    <p className={'T05 P25'}>{item.BRANCH_NAME}</p>
                    <p className={'T05 P30'}><Link scroll={false} href={`/setting/admins/edit/?id=${item.derived_ADMIN_id}`}><span className={'isStatus'}>수정하기</span>{item.USER_NUMBER ? <span className={'isMini'}>EIN: {item.USER_NUMBER}</span> : null}</Link></p>
                  </div>
                })}
              </div>
              <div className={'C04'}>
                <p className={'T06 styleSheet isFirst'} onClick={e => {
                  router.push(`/setting/admins/`, {scroll: false});
                  setIndexBoard(1);
                }}></p>
                <p className={'T06 styleSheet isPrev'} onClick={e => {
                  if(indexBoard > 10) {
                    router.push(`/setting/admins/?index=${Math.floor((indexBoard - 1) / 10)*10}`, {scroll: false});
                    setIndexBoard(Math.floor((indexBoard - 1) / 10)*10);
                  } else {
                    router.push(`/setting/admins/?index=1`, {scroll: false});
                    setIndexBoard(1);
                  }
                }}></p>
                <div className={'C05'}>
