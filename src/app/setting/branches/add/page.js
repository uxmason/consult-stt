...
export default function SettingBranchesAdd() {
  return (
...
function AddContent() {
  ...
  const boostAddData = async () => {
    if(!isLoader) {
      try {
        setLoader(true);
        let url = `/api/setting/branches/add`;
        const insert = await fetch(url, {
          ...
          body: JSON.stringify({ textName: textName, textCode: textCode}),
        });
        ...
        <div className={styles.C01} style={{height: dimensions.height+40+'px'}}>
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C04}>
              ...
              <p className={'T10'}>신규 지점 추가하기</p>
            </div>
            <div className={styles.C05}>
              <p className={styles.T01}>신규 지점 코드</p>
              <input className={styles.I00} maxLength={2} onChange={e => {
              ...
              <p className={styles.T01}>신규 지점 이름</p>
              ...
              <div className={styles.C07} >
                <p className={styles.T00} onClick={e => {
                  if (isValidated0 && isValidated1) boostAddData();
                }}>추가하기</p>
              </div>
            </div>
          </div>
