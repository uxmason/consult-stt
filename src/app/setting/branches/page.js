...
export default function SettingBranches() {
  return (
...
function Content() {
  ...
  const boostListData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/branches/?index=${indexBoard}`;
      if(indexBoard == null) url = `/api/setting/branches/`;
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C05}>
              <div className={'C02 isOpacity isMini isUnderline isShort'}>
                <p className={'T05 P10'}>코드</p>
                <p className={'T05 P30'}>지점명</p>
                <p className={'T05 P25'}>설정</p>
                <p className={'T05 P35'}>최종 수정일</p>
              </div>
              <div className={'C05'}>
                {data && data.map((item, index) => {
                  return <div key={index} className={'C02 isPrevented'}>
                    <p className={'T05 P10'}>{item.BRANCH_INFO_CODE}</p>
                    <p className={'T05 P30'}>{item.BRANCH_NAME}</p>
                    <Link scroll={false} href={`/setting/branches/edit/?id=${item._id}`}>
                      <p className={'T05 P25'}><span className={'isStatus'}>수정하기</span></p>
                    </Link>
                    {item.updatedAt 
                      ? <p className={'T05 P35 isGray isMini'}>{dateFormatYYMMDD_HHMM(new Date(new Date(item.updatedAt).getTime() + (new Date(item.updatedAt).getTimezoneOffset() * 60 * 1000)))}</p>
                      : <p className={'T05 P35 isGray isMini'}>{dateFormatYYMMDD_HHMM(new Date(new Date(item.createdAt).getTime() + (new Date(item.createdAt).getTimezoneOffset() * 60 * 1000)))}</p>}
                  </div>
                })}
              </div>
              <div className={'C04'}>
              ...
              <div>
                <Link scroll={false} href="/setting/branches/add/"><p className={styles.T00}>지점 추가</p></Link>
              </div>
            </div>
          </div>
