...
export default function SettingRanksEdit() {
  return (
 ...
 function EditContent() {
  ...
  const boostGetData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/rank/${searchParams.get('id')}`;
      const insert = await fetch(url, {
        method: 'GET',
      });
      const content = await insert.json();
      if(content.success) {
        setTextPosition(content.data.RANK_NAME);
        setLoader(false);
      }else {
        setLoader(false);
        ...
        <div className={styles.C01} style={{height: dimensions.height+40+'px'}}>
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C04}>
              ...
              <p className={'T10'}>관리자 직급 수정하기</p>
            </div>
            <div className={styles.C05}>
              <p className={styles.T01}>직급 이름</p>
