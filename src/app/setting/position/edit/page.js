...
export default function SettingPositionsEdit() {
  return (
...
function EditContent() {
  ...
  useEffect(() => {
    if(textPosition) {
      (textPosition.length > 1) ? setValidated0(true) : setValidated0(false);
    }
  }, [textPosition])
  ...
  const boostGetData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/position/${searchParams.get('id')}`;
      const insert = await fetch(url, {
        method: 'GET',
      });
      const content = await insert.json();
      if(content.success) {
        setTextPosition(content.data.POSITION_NAME);
        setLoader(false);
      }else {
        setLoader(false);
  ...
  const boostEditData = async () => {
    if(!isLoader) {
      try {
        setLoader(true);
        let url = `/api/setting/position/edit`;
        const insert = await fetch(url, {
          method: 'PUT',
          headers: {
        ...
        <div className={styles.C01} style={{height: dimensions.height+40+'px'}}>
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C04}>
              ...
              <p className={'T10'}>관리자 직책 수정하기</p>
            </div>
            <div className={styles.C05}>
              <p className={styles.T01}>직책 이름</p>
              <input className={styles.I00} maxLength={8} defaultValue={textPosition} onChange={e => {
                (e.currentTarget.value.length > 1) ? setValidated0(true) : setValidated0(false);
                setTextPosition(e.currentTarget.value);
              }} />
