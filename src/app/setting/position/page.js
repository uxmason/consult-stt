...
export default function SettingPositions() {
  return (
...
function Content() {
  ...
  const boostListData = async () => {
    try {
      setLoader(true);
      let url = `/api/setting/position/?index=${indexBoard}`;
      if(indexBoard == null) url = `/api/setting/position/`;
            ...
            <div className={styles.C05}>
              ...
              <div className={'C05'}>
                {data && data.map((item, index) => {
                  return <div key={index} className={'C02 isPrevented'}>
                    <p className={'T05 P30'}>{item.POSITION_NAME}</p>
                    <Link scroll={false} href={`/setting/position/edit/?id=${item._id}`}>
              ...
              <div className={'C04'}>
                <p className={'T06 styleSheet isFirst'} onClick={e => {
                  router.push(`/setting/position/`, {scroll: false});
                  setIndexBoard(1);
                }}></p>
                <p className={'T06 styleSheet isPrev'} onClick={e => {
                  if(indexBoard > 10) {
                    router.push(`/setting/position/?index=${Math.floor((indexBoard - 1) / 10)*10}`, {scroll: false});
                    setIndexBoard(Math.floor((indexBoard - 1) / 10)*10);
                  } else {
                    router.push(`/setting/position/?index=1`, {scroll: false});
                    setIndexBoard(1);
                  }
                }}></p>
                <div className={'C05'}>
                  {boardNumbers.map((item, index) => {
                    return (<p key={index} className={`T06 isPager ${(indexBoard == item.number)? `isSelected`:null}`} onClick={e => {
                      router.push(`/setting/position/?index=${item.number}`, {scroll: false});
                      setIndexBoard(item.number);
                    }}>{item.number}</p>)
                  })}
                </div>
                {Math.ceil(indexBoard / 10) != Math.ceil(totalCount / 100) ? <><p className={'T06 styleSheet isNext'} onClick={e => {
                  router.push(`/setting/position/?index=${Math.ceil(indexBoard / 10)*10 + 1}`, {scroll: false});
                  setIndexBoard(Math.ceil(indexBoard / 10)*10 + 1);
                }}></p><p className={'T06 styleSheet isEnd'} onClick={e => {
                  router.push(`/setting/position/?index=${Math.ceil(totalCount / 10)}`, {scroll: false});
                  setIndexBoard(Math.ceil(totalCount / 10));
                }}></p></> : null}
              </div>
              <div>
                <Link scroll={false} href="/setting/position/add/"><p className={styles.T00}>직책 추가</p></Link>
              </div>
            </div>
          </div>
