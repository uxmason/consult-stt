...
export default function SettingPositionsAdd() {
  return (
...
function AddContent() {
  ...
  const boostAddData = async () => {
    if(!isLoader) {
      try {
        setLoader(true);
        let url = `/api/setting/position/add`;
        const insert = await fetch(url, {
          ...
          body: JSON.stringify({ textPosition: textPosition }),
        });
        ...
        <div className={styles.C01} style={{height: dimensions.height+40+'px'}}>
          ...
          <div className={styles.C02} ref={targetRef}>
            <div className={styles.C04}>
              ...
              <p className={'T10'}>관리자 직책 추가하기</p>
            </div>
            <div className={styles.C05}>
              <p className={styles.T01}>직책 이름</p>
              <input className={styles.I00} maxLength={8} onChange={e => {
