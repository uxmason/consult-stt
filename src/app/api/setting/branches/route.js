...
export async function GET(request) {
  try {
    ...
    const data = await ExecuteQuery(`SELECT * FROM CORE_BRANCHES ORDER BY createdAt DESC OFFSET ${(count-1)*10} ROWS FETCH NEXT 10 ROWS ONLY`);
    const totalCount = await ExecuteQuery(`SELECT COUNT(*) AS count FROM CORE_BRANCHES`);
