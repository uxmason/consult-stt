...
export default function Consult() {
  ...
  const boostListData = async () => {
    try {
      setLoader(true);
      let url = `/api/emrConsult/`;
      if(positionID != 11) url = `/api/emrConsult/list/${userID}/`;
      if(indexBoard != null) {
            ...
            <div className={styles.C04}>
              ...
              <div className={'C01'}>
                ...
                <p className={'T02'} onClick={e => {
                  router.push(`/emrConsult/?index=1&psEntry=${psEntry}&psName=${psName}&dateFrom=${dateFrom}&dateTo=${dateTo}&sorting=${sorting}`, {scroll: false});
                  boostListData();
                }}>검색하기</p>
              </div>
            </div>
            <div className={styles.C05}>
              ...
              <div className={'C04'}>
                ...
                <p className={'T06 styleSheet isPrev'} onClick={e => {
                  if(indexBoard > 10) {
                    router.push(`/emrConsult/?index=${Math.floor((indexBoard - 1) / 10)*10}&psEntry=${psEntry}&psName=${psName}&dateFrom=${dateFrom}&dateTo=${dateTo}&sorting=${sorting}`, {scroll: false});
                ...
                <div className={'C05'}>
                  {boardNumbers.map((item, index) => {
                    return (<p key={index} className={`T06 isPager ${(indexBoard == item.number)? `isSelected`:null}`} onClick={e => {
                      router.push(`/emrConsult/?index=${item.number}&psEntry=${psEntry}&psName=${psName}&dateFrom=${dateFrom}&dateTo=${dateTo}&sorting=${sorting}`, {scroll: false});
                      setIndexBoard(item.number);
                    }}>{item.number}</p>)
                  })}
                </div>
                {Math.ceil(indexBoard / 10) != Math.ceil(totalCount / 100) ? <><p className={'T06 styleSheet isNext'} onClick={e => {
                  router.push(`/emrConsult/?index=${Math.ceil(indexBoard / 10)*10 + 1}&psEntry=${psEntry}&psName=${psName}&dateFrom=${dateFrom}&dateTo=${dateTo}&sorting=${sorting}`, {scroll: false});
                  setIndexBoard(Math.ceil(indexBoard / 10)*10 + 1);
                }}></p><p className={'T06 styleSheet isEnd'} onClick={e => {
                  router.push(`/emrConsult/?index=${Math.ceil(totalCount / 10)}&psEntry=${psEntry}&psName=${psName}&dateFrom=${dateFrom}&dateTo=${dateTo}&sorting=${sorting}`, {scroll: false});
